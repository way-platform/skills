...
```proto
rpc BatchDeleteBooks(BatchDeleteBooksRequest) returns (aep.api.Operation) {
  option (google.api.http) = {
    post: "/v1/{parent=publishers/*}/books:batchDelete"
    body: "*"
  };
...
- The response message **should** be `google.protobuf.Empty` for atomic hard-deletes.
  - If the resource is [soft deleted][soft-delete], OR if the method supports synchronous partial success, the response message **should** be a custom response message (e.g., `BatchDeleteBooksResponse`).
...
- The batch delete method **may** support atomic (all resources deleted or none
  ...
  - **End-User Experience:** Consider the perspective of the API consumer.
    ...
    entries?
- Both synchronous and asynchronous batch delete **may** support atomic or partial success.
...
message BatchDeleteBooksRequest {
  ...
  // The paths of the books to delete.
...
- A `parent` field **should** be included, unless the resource being deleted is
  ...
  parent collection in the path of any resource being deleted does not match,
...
- The request message **must** include a repeated field which accepts the
  resource paths specifying the resources to delete. The field **should** be
  named `paths`.
  - The field **should** be required.
  - The field **should** identify the [resource type][aep-122-paths] that it
    references.
  - The comment for the field **should** document the resource pattern.
- Other fields besides `path` **may** be "hoisted" from the [standard Delete
...
besides the resource path that needs to be different between different
...
- Filter-based matching **must not** be supported unless it is infeasible to
  ...
  to accidentally delete important data. If it is unavoidable, see [AEP-165][].

### Response message

In cases where a response message is necessary (e.g., because the resource is soft-deleted, or the method supports synchronous partial success), the response **should** be specified with the following pattern:

```proto
message BatchDeleteBooksResponse {
  // Books deleted. (Optional, typically used for soft-delete)
  repeated Book results = 1;

  // Optional: Communicates partial failures if the method supports partial success.
  // The key is the index of the request in the `requests` (or `paths`) field.
  map<int32, google.rpc.Status> failed_requests = 2;
}
```

- If returning updated resources (e.g., soft-delete), the response message **must** include one repeated field corresponding to the resources that were deleted.
- If the batch delete method supports partial success, the response message **must** include a `map<int32, google.rpc.Status> failed_requests` field to communicate the partial failures.
...
- **2020-09-16**: Suggested annotating `parent`, `paths`, and `requests` fields.
- **2020-08-27**: Removed parent recommendations for top-level resources.
- **2020-03-27**: Added reference to AEP-165 for criteria-based deletion.

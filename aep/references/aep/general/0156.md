...
- Singleton resources **may** define the [`List`][aep-132] method, but **must**
  implement it according to AEP-159. See the example below.
  ...
  - If a parent resource ID is provided instead of the hyphen `-` as per
    AEP-159, then the service **should** return a collection of one Singleton
...
message ListConfigsRequest {
  ...
  string parent = 1 [
    (aep.api.field_info) = { resource_reference: [ "api.example.com/config" ], field_behavior: [ FIELD_BEHAVIOR_REQUIRED ] }];

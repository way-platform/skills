...
page_title: "Apache Snowflake I/O connector"
...
- [SnowflakeIO Javadoc](https://beam.apache.org/releases/javadoc/2.71.0/org/apache/beam/sdk/io/snowflake/SnowflakeIO.html)
...
Use Beamâs [Pipeline options](https://beam.apache.org/releases/javadoc/current/org/apache/beam/sdk/options/PipelineOptions.html) to set options via the command line.
...
./gradle run
    --args="
        ...
        --query=<IF NOT TABLE THEN QUERY>
           Example: --query=âSELECT column FROM TABLEâ
...
Where all parameters are starting with â–â, they are surrounded with double quotation and separated with comma:
...
- `--schema=<SNOWFLAKE SCHEMA>`
  - Specifies the schema to use for the specified database once connected. The specified schema should be an existing schema for which the specified userâs role has privileges.
  - Example: `--schema=PUBLIC`
...
- `--database=<SNOWFLAKE DATABASE>`
  - Specifies the database to use once connected. The specified database should be an existing database for which the specified userâs role has privileges.
...
- `--runner=DataflowRunner`
  - The Dataflowâs specific runner.
...
**Note**:
SnowflakeIO uses `COPY` statements behind the scenes to write (using [COPY to table](https://docs.snowflake.net/manuals/sql-reference/sql/copy-into-table.html)). StagingBucketName will be used to save CSV files which will end up in Snowflake. Those CSV files will be saved under the âstagingBucketNameâ path.

**Optional** for batching:

- `.withQuotationMark()`
  - Default value: `â` (single quotation mark).
  - Accepts String with one character. It will surround all text (String) fields saved to CSV. It should be one of the accepted characters by [Snowflakeâs](https://docs.snowflake.com/en/sql-reference/sql/create-file-format.html) [FIELD_OPTIONALLY_ENCLOSED_BY](https://docs.snowflake.com/en/sql-reference/sql/create-file-format.html) parameter (double quotation mark, single quotation mark or none).
...
SnowflakeIO is not going to delete created CSV files from path under the âstagingBucketNameâ either during or after finishing streaming.
...
The CSVMapperâs job is to give the user the possibility to convert the array of Strings to a user-defined type, ie. GenericRecord for Avro or Parquet files, or custom POJO.
...
- [SnowflakeIO Pydoc](https://beam.apache.org/releases/pydoc/2.71.0/apache_beam.io.snowflake.html)
- [Snowflake documentation](https://docs.snowflake.com/en)
...
Itâs required to pass one of the following combinations of valid parameters for authentication:
...
- `schema` Name of the Snowflake schema in the database to use.
- `database` Name of the Snowflake database to use.
- `staging_bucket_name` Path to Google Cloud Storage bucket or AWS S3 bucket ended with slash. Bucket will be used to save CSV files which will end up in Snowflake. Those CSV files will be saved under âstaging_bucket_nameâ path.
- `storage_integration_name` Is the name of a Snowflake storage integration object created according to [Snowflake documentation](https://docs.snowflake.net/manuals/sql-reference/sql/create-storage-integration.html).
...
Itâs required to pass one of the following combination of valid parameters for authentication:

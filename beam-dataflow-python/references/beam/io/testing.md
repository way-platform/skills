...
page_title: "Testing I/O Transforms"
beam_last_updated: "Last updated on 2026/03/06"
---

## Testing I/O Transforms in Apache Beam

_Examples and design patterns for testing Apache Beam I/O transforms_
...
If you are using the `Source` API, make sure to exhaustively unit-test your code. A minor implementation error can lead to data corruption or data loss (such as skipping or duplicating records) that can be hard for your users to detect. Also look into using `source_test_utils` - it is a key piece of testing `Source` implementations.

If you are not using the `Source` API, you can use `TestPipeline` with `assert_that` to help with your testing.
...
1. Set up the data store corresponding to the test you wish to run. You can find Kubernetes scripts for all currently supported data stores in [.test-infra/kubernetes](https://github.com/apache/beam/tree/master/.test-infra/kubernetes).
   ...
   2. Convention dictates there will be:
      ...
      2. A separate script, with LoadBalancer service. Such service will expose an _external ip_ for the datastore. Such scripts are needed when external access is required (eg. on Jenkins).
   3. Examples:
...
- Small scale
  - Execute on a single worker on the runner (it should be _possible_ but is not required).

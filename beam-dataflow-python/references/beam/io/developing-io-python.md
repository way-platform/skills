...
page_title: "Apache Beam: Developing I/O connectors for Python"
...
To connect to a data store that isnât supported by Beamâs existing I/O
...
This guide covers using the [Source and FileBasedSink interfaces](https://beam.apache.org/releases/pydoc/2.71.0/apache_beam.io.iobase.html)
...
1. **Serializability:** Your `Source` or `FileBasedSink` subclass must be
   ...
   facilitate reading or writing in parallel. The _way_ the source and sink
...
[apache_beam.io.iobase module](https://beam.apache.org/releases/pydoc/2.71.0/apache_beam.io.iobase.html).
...
- `estimate_size`: Services use this method to estimate the _total size_ of your data, in bytes. This estimate is in terms of external storage size, before performing decompression or other processing.
...
- **Position-based sources** - A position-based source can be described by a range of positions of an ordered type, and the records read by the source can be described by positions of that type. For example, for a record within a file, the position can be the starting byte offset of the record. The position type for the record in this case is `long`.
  ...
  The concept of _split points_ allows to extend the definitions for dealing with sources where some records cannot be identified by a unique starting position.
...
- **Split points** - A split point describes a record that is the first one returned when reading the range from and including position **A** up to infinity (i.e. [A, infinity)).
  ...
  To support such complex formats, Beam introduces the notion of _split points_. A record is a split point if there exists a position **A** such that the record is the first one to be returned when reading the range [A, infinity). In CBF, the only split points would be the first records in each block.
...
- **Consumed positions** - Consumed positions refer to records that have been read.
  ...
  As the source is being read, and records read from it are being passed to the downstream transforms in the pipeline, we say that positions in the source are being _consumed_. When a reader has read a record (or promised to a caller that a record will be returned), positions up to and including the record’s start position are considered _consumed_.

  Dynamic splitting can happen only at _unconsumed_ positions. If the reader just returned a record at offset 42 in a file, dynamic splitting can happen only at offset 43 or beyond. Otherwise, that record could be read twice (by the current reader and the reader of the new task).
...
If your data source uses files, you can implement the [FileBasedSink](https://beam.apache.org/releases/pydoc/2.71.0/apache_beam.io.filebasedsink.html)
...
For example, if your usersâ pipelines read from your source using
...
See Beamâs [PTransform style guide](/contribute/ptransform-style-guide/#exposing-a-ptransform-vs-something-else)

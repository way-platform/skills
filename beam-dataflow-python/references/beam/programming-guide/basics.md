...
page_title: "Basics of the Beam model"
...
- [_Watermark_](#watermark) - A watermark is a guess as to when all data in a
  certain window is expected to have arrived. This is needed because data isnât
...
object, and then uses that object as the basis for creating the pipelineâs data
...
(colloquially referred to as âuser codeâ), and your user code is applied to each
...
your pipelineâs PCollections. These include general-purpose core transforms,
...
fit your pipelineâs exact use case.
...
![Aggregation of elements.](/images/aggregation.png)

_Figure 1: Aggregation of elements. Elements with the same color represent those
with a common key and window._
...
a data event occurs (the âevent timeâ, determined by the timestamp on the data
element itself) and the time the actual data element gets processed at any stage
in your pipeline (the âprocessing timeâ, determined by the clock on the system
processing the element). In addition, data isnât always guaranteed to arrive in
...
arrived in the pipeline. You can also think of this as âweâll never see an
element with an earlier timestampâ.
...
âinfinityâ. In this manner, you might discover that an unbounded `PCollection`
...
_pane_). If you use Beamâs default windowing configuration and default trigger,
...
- **Event time triggers**: These triggers operate on the event time, as
  indicated by the timestamp on each data element. Beamâs default trigger is
  event time-based.
...
Beamâs windowing and triggers provide an abstraction for grouping and

...
may observe various effects as a result of the runnerâs choices. This page
...
The way in which runners _shuffle_ data may be slightly different for Batch and
...
downstream `PCollection`. Finally, consider two events _with the same key_
...
_Figure 1: A runner divides an input collection into two bundles._
...
_Figure 2: Two workers process the two bundles in parallel._
...
_Figure 3: Nine workers process a nine element input collection in parallel._
...
_Figure 4: Two transforms in sequence and their corresponding input collections._
...
_Figure 5: Two workers execute dependently parallel ParDo transforms._
...
elements were successfully processed, the third elementâs processing failed, and
...
_Figure 6: The processing of an element within bundle B fails, and another worker
retries the entire bundle._
...
_Figure 7: Processing of an element within bundle D fails, so all elements in
the input bundle are retried._
...
down since they arenât following the normal `DoFn` lifecycle .
